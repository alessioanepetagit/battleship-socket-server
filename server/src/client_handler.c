...
/* MODIFICA: scorciatoia per mandare un errore, cosi' non ripetiamo
...
/*
 * MODIFICA CENTRALE: una sola funzione che smonta la partita corrente.
...
static void handle_create_game(GameManager *gm, Player *player) {
    ...
    if (!g) {
        build_response(RSP_ERROR, resp, sizeof(resp), 2, "500", "Limite massimo partite raggiunto");
        gm_notify_player(player, resp);
        return;
    }
...
static void handle_list_games(GameManager *gm, Player *player) {
    ...
    /* MODIFICA: GameInfo al posto della copia dell'intera struct Game */
...
static void handle_join_game(GameManager *gm, Player *player, const Message *msg) {
    ...
    /* MODIFICA: si puo' chiedere di entrare solo se si e' liberi in lobby */
    if (get_player_state(player) != PLAYER_LOBBY) {
        send_error(player, ERR_WRONG_STATE);
        return;
    }
    ...
    /* MODIFICA: il richiedente resta "in attesa di risposta" */
    pthread_mutex_lock(&player->lock);
...
/* Risolve il giocatore indicato dal creatore: accetta sia l'id che lo username */
...
static void handle_reject_invite(GameManager *gm, Player *player, const Message *msg) {
    ...
    pthread_mutex_lock(&g->lock);
    /* MODIFICA: stesso controllo dell'accept */
    if (g->creator_id != player->id || g->pending_invite_from != requester->id) {
        pthread_mutex_unlock(&g->lock);
        send_error(player, ERR_NO_PENDING_INVITE);
        return;
    }
    g->pending_invite_from = -1;   /* MODIFICA: prima non veniva mai azzerato */
    pthread_mutex_unlock(&g->lock);
...
static void handle_fire(GameManager *gm, Player *player, const Message *msg) {
    ...
    /*
     * MODIFICA: protezione contro il caso opponent == player.
    ...
    bool victory = check_victory(&opponent->board);
    ...
    if (victory) {
        ...
        /*
         * MODIFICA: a fine partita i due giocatori passano in
         * PLAYER_FINISHED (prima restavano in PLAYER_IN_GAME e non
         * potevano piu' fare nulla). Da qui possono chiedere la rivincita
         * oppure uscire, come richiede la traccia.
         */
        pthread_mutex_lock(&player->lock);
...
/*
 * MODIFICA: LEAVE_GAME. Esce dalla partita ma NON chiude la connessione,
 * cosi' il giocatore torna davvero nella lobby (prima QUIT_GAME chiudeva
 * il socket mentre il client mostrava il menu della lobby).
 */
...
/*
 * MODIFICA: la rivincita richiesta dalla traccia.
